...
std::string GameObject::GetChildrenNames(GameObject* pGameObject, int pDepth) const {
	std::string childNames = "Children of " + getName() + ": \n";
	...
	for (int i = 0; i < childCount; i++) {
		//std::string padding = "";
		//padding.insert(padding.begin(), pDepth - padding.size(), ' ');

		child = pGameObject->getChildAt(i);
		childNames += /*padding +*/ "\t" + child->getName() + "\n";

		if (childCount > 0) {
			GetChildrenNames(child, pDepth++);
		}
	}
