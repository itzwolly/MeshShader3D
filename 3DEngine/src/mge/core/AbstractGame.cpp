...
void AbstractGame::run()
{
    ...
    FPSBenchmarkTool* fpsBenchmark = new FPSBenchmarkTool();
    sf::Clock benchmarkClock;
    float timeSinceLastBenchmarkUpdate = 0;
    float initialWaitTimeBenchmark = 5;
    // bool runTest = true;

    // Delta time for uncapped fps
    float deltaTime = 0;
	while (_window->isOpen()) {
		...
		// if (timeSinceLastUpdate > timePerFrame) {
            ...
            // Timer used to keep track of the amount of time to run a benchmark test
            timeSinceLastBenchmarkUpdate += benchmarkClock.restart().asSeconds();

            // Benchmarking each frame instead of every second (below)
            //if (runTest) {
            //    if (_fps > 0) {
            //        fpsBenchmark->updateManually(_fps);
            //    }

            //    if (fpsBenchmark->getCount() == 300) { // taking the average fps over 300 entries (same as the tests in unreal engine)
            //        runTest = false;
            //    }
            //} else {
            //    int benchmarkCount = fpsBenchmark->stop();
            //    float averageFps = fpsBenchmark->getAverageFps();

            //    if (benchmarkCount > 0) {
            //        std::cout << "[FPSBenchmarkTool] Average FPS is: " << averageFps << " (" << 1000.0f / averageFps << " ms per frame) average taken over: " << benchmarkCount << " entries" << std::endl;
            //    }
            //}
            ...
            if (timeSinceLastFPSCalculation > 1) {
                ...
                if (timeSinceLastBenchmarkUpdate > initialWaitTimeBenchmark) { // wait an initial x amount of seconds to make sure everything in the scene is loaded
                    //Update the fps benchmark tool
                    fpsBenchmark->update(_fps, 20);
                }
